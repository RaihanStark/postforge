// Auto-update PostForge from its GitHub Releases.
//
// electron-updater checks the `publish` target configured in package.json (the
// RaihanStark/postforge GitHub repo), compares the latest published release to
// the running version, and — if a newer one exists — downloads the matching
// AppImage in the background and installs it on the next quit. The update
// metadata (latest-linux.yml + the AppImage) is produced by electron-builder and
// attached to each release by the Release workflow.
//
// Notes:
//   - Auto-update only works in a packaged build. In dev (`npm run electron`) the
//     app isn't packaged and electron-updater has no feed, so we no-op.
...
export function initAutoUpdater(win) {
  ...
  autoUpdater.on('update-downloaded', async (info) => {
    ...
    const { response } = await dialog.showMessageBox(win, {
      ...
      cancelId: 1,
      title: 'Update ready',
      message: `PostForge ${info?.version} has been downloaded.`,
